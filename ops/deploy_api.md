...
- (선택) 가입 승인 안내 메일: `SMTP_HOST`, `SMTP_PORT`(587 STARTTLS), `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM_EMAIL`, `SMTP_FROM_NAME`, `SMTP_USE_TLS`, `PUBLIC_SITE_URL`, `RATE_LIMIT_ACTIVATION_EMAIL`. staging/prod에서 SMTP 자격증명이 있으면 `SMTP_USE_TLS=true`가 필수다. `PUBLIC_SITE_URL`은 staging/prod에서 공개 https 도메인이어야 하며, 비우거나 localhost로 두면 기동 실패한다. 로컬 기본값은 `http://localhost:3000`이다.

...
function SectionHeader({
  ...
  return (
    <div className="flex items-center justify-between">
      <h3 className="font-kopub text-base text-neutral-ink">{title}</h3>
      <Link
        href={href}
        className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-primary transition-colors hover:bg-brand-primaryDark focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
        ...
        <div className="space-y-1">
          {events.map((evt) => (
            <Link
              key={evt.id}
              href={`/events/${evt.id}`}
              className="flex min-h-11 items-start justify-between gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-surface/70 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
            >
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {Array.from({ length: 4 }, (_, i) => (
        <div
          key={`news-skeleton-${i}`}
          className={`aspect-4/3 rounded-2xl bg-neutral-subtle ${i > 1 ? 'hidden lg:block' : ''}`}
        />
      ))}
    </div>
  );
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {(posts ?? []).map((post, idx) => (
            <Link
              key={post.id}
              href={`/posts/${post.id}`}
              className={`group relative overflow-hidden rounded-2xl border border-neutral-border bg-white shadow-sm transition-shadow hover:shadow-md focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 ${
...
export function HomeDesktopCards() {
  ...
  return (
    <section aria-labelledby="home-activity" className="mt-10 space-y-6">
      <div>
        <p className="text-xs font-semibold text-brand-700">지금 동문회에서는</p>
        <h2 id="home-activity" className="mt-1 text-xl font-semibold text-text-primary">최근 활동</h2>
      </div>

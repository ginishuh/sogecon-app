  ...
  return (
    ...
    >
      <section className="mx-auto max-w-4xl space-y-4 p-6">
        ...
        <p hidden={!query.isLoading} className="text-sm text-text-secondary">
          불러오는 중…
        </p>
        <p hidden={!query.isError} className="text-sm text-state-error">
          목록을 불러오지 못했습니다.
        </p>

        <div hidden={!showEmpty} className="rounded border border-neutral-border bg-white p-6 text-sm text-text-secondary">
          등록된 배너가 없습니다. 게시물/행사 관리에서 배너를 켜주세요.
        </div>

        <ul hidden={!hasItems} className="divide-y rounded border border-neutral-border bg-white">
          {items.map((item) => (
            <li key={item.id} className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0 space-y-1">
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  <span className="font-medium text-text-primary">{targetLabel(item)}</span>
                  <a
                  ...
                  {!item.enabled ? (
                    <span className="rounded bg-neutral-subtle px-2 py-0.5 text-xs text-text-secondary">비노출</span>
                  ) : null}
                  {item.pinned ? (
                    <span className="rounded bg-state-warning-subtle px-2 py-0.5 text-xs text-state-warning">상단 고정</span>
                  ) : null}
                </div>
        ...
        <div hidden={!hasItems} className="flex justify-between pt-2">
          <button
            type="button"
            className="rounded border px-3 py-2 text-sm text-text-secondary hover:bg-surface-raised disabled:opacity-40"

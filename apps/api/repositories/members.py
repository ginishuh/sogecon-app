...
def _clear_directory_consent_if_private(
...
async def update_member_profile_admin(
    ...
    if updates:
        ...
        if isinstance(next_visibility, models.Visibility):
            _clear_directory_consent_if_private(member, next_visibility)
        await db.commit()
        await db.refresh(member)
    return member
...
async def save_directory_consent(
    ...
    *,
    ...
    _clear_directory_consent_if_private(member, visibility)
    await db.commit()
    await db.refresh(member)
    return member

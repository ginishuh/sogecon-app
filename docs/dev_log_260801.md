...
- SSR `apiFetch`가 `next/headers` 쿠키를 API로 전달해 관리자 draft `/posts/{id}` 미리보기가 익명 404로 닫히지 않게 함.
- board 검색 `maxLength={100}` + `q` 슬라이스, `keepPreviousData`로 검색 전환 스켈레톤 깜빡임 완화.
- create/update 시 `KNOWN_POST_CATEGORIES` 화이트리스트(미지·누락 → `422 invalid_post_category`).

...
>   ※ **정합성 제안**: 공식 생년월일(read-only)과 별개로 ‘표시용 생년월일(양/음)’을 두어 동문수첩 생일 표시에 활용
> * 연락처(휴대폰)
> * 이메일
> * 직장명 / 부서명 / 직장전화번호
> * 개인주소 / 직장주소
...
* **일반회원**: 로그인, 내 정보 수정, 디렉토리/소식/소개/커뮤니티 열람, 게시판 글/댓글 작성(정책에 따라 승인/레벨 제한 가능)
...
2. **디렉토리 탐색** → 검색/필터 → 카드 확장 → 원클릭 연락/저장
...
* **헤더 고정** / **바텀 탭(선택)**: 홈·수첩·일정·커뮤니티·내 계정
* **검색바(전역/국지)**: 전역 검색은 v2 고려, 우선 수첩/공지/게시판에 국지 검색
...
* **완결성**: 로그인/활성화/대시보드/수첩/소식/소개/커뮤니티/행사/문의/관리자 콘솔 **전체 흐름 정의**
* **검증성**: 목록/상세/검색·필터/권한/공개범위/알림 동작 시나리오 명시
* **가독성**: 메뉴 트리와 화면 구분, 2×2/프리뷰/슬라이드 구조 명확
* **일관성**: 용어/탭/버튼명 통일(“총원우회”로 일괄), 날짜·시간 **YYYY-MM-DD / 24h / Asia/Seoul**
...
1. **와이어프레임(저충실)**: 대시보드/수첩 목록·상세/행사 목록·상세/공지·게시판/내 정보 · 총 8화면

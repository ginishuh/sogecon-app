...
## 3) 토큰(Token)
Tailwind CSS 4 설정(`apps/web/app/styles/tailwind-theme.css`의 `@theme`)에서 semantic 레이어로 정의합니다.
...
- [ ] 200% 확대에서도 정보·조작 손실 없이 reflow (Windows visual E2E 완료 후 확정)
...
### 체크리스트
- [x] `tailwind-theme.css`의 `@theme`에 semantic 토큰 정의(AA 대비 준수)

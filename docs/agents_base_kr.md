...
## VPS 운영 원칙 (/srv)
- 배포/운영 명령은 대상 리포의 `/srv/<repo>` 경로에서 직접 실행합니다.
- 작업 시작 전 `pwd`와 `git remote -v`로 리포/원격을 확인합니다.
- 서로 다른 리포의 배포 스크립트, Compose 파일, 환경 파일을 혼용하지 않습니다.
- `.env`, 키 파일, 인증서, 토큰, DB 백업은 절대 커밋하지 않습니다.
- 공개 서비스는 가능하면 `127.0.0.1`에 바인딩하고 외부 노출은 Nginx/리버스 프록시에서 처리합니다.
- 위험한 운영 변경 전에는 백업/스냅샷과 롤백 경로를 먼저 확인합니다.
- 배포 후에는 같은 리포 기준으로 서비스 상태, 헬스체크, 최근 로그를 검증합니다.
- 공통 배포 후 점검 도구: `/root/scripts/post_deploy_check.sh <repo-name|repo-path> [health_url ...]`.
- 장애 보고는 `현상 → 영향 → 조치 → 검증 → 재발 방지` 순서로 작성하고, `2026-04-26 14:30 KST`처럼 절대 시간을 사용합니다.
...
### 실행 절차 원칙(신중함 > 속도)
- 이 원칙은 비단순 작업에서 엄격 적용하고, 사소한 작업은 경량 적용한다.
- 코딩 전 사고: 가정을 명시하고, 모호하면 질문해 확인한다.
- 해석이 여러 개인 요청은 임의 선택하지 말고 선택지를 제시한다.
- 가장 단순한 유효 해법을 우선하고, 추측성 기능/추상화는 넣지 않는다.
- 변경은 외과적으로: 요청 달성에 필요한 범위만 수정한다.
- 인접한 비관련 코드의 리팩터/포맷/정리는 하지 않는다.
- 이번 변경으로 생긴 미사용 코드/임포트만 정리한다.
- 요청을 검증 가능한 목표로 변환한다(예: 재현 -> 수정 -> 테스트 통과).
- 다단계 작업은 단계별 검증이 붙은 짧은 계획을 유지한다.
- 성공 조건은 독립적으로 확인 가능한 수준으로 구체화한다.
...
- Serena MCP(코드 내비게이션):
  ...
  - Codex repo-local `.codex/config.toml`: `--project /home/ginis/sogecon-app --context codex --mode interactive --mode editing --mode no-onboarding`으로 이 checkout에 바인딩합니다.
  - 공유/전역 MCP config에는 이 리포 경로를 박지 않습니다. 공유 Serena 서버가 `--project-from-cwd`로 시작된 경우에는 세션 시작 시 한 번 `activate_project`를 호출합니다.

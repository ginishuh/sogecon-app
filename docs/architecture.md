...
- RUM: Web Vitals 5가 `LCP/INP/CLS/FCP/TTFB`의 `id`, `value`, `delta`, `rating`, `navigationType`을 `POST /rum/vitals`로 전송한다. 경로는 query를 제거한 pathname만 포함하고 이메일·학번·토큰 등 개인정보는 수집하지 않는다. 구 클라이언트의 롤링 배포를 위해 `rating`과 `navType`은 API에서 선택 항목으로 수용하며, beacon/fetch 실패는 사용자 흐름이나 오류 추적 이벤트를 만들지 않는다.

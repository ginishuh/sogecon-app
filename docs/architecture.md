...
| `Post` | `author_id`, `title`, `content`, `published_at`, `category`, `view_count` | 공지/게시글. 공개 API는 board 카테고리(`discussion`/`question`/`share`/`congrats`)는 `published_at` 무관 공개, `notice`/`news` 등은 `published_at <= now`만 공개. 생성·수정 `category`는 위 6종만 허용. `view_count`는 서버 전용(생성 입력 불가). 공개 목록은 `q` 서버 검색을 지원. 상세 SSR은 세션 쿠키를 API로 전달해 관리자 draft 미리보기를 유지. |

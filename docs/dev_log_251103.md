...
### 로고 에셋 파일명 교정
- `seogang.svg`, `seogang_korean_logo.svg` → `sogang.svg`, `sogang_korean_logo.svg`
### OPS 기본 경로 통일
...
### CI 빌드 트리거 필터링
- `build-push.yml`에 path 필터 도입 → `push(main)`은 `apps/**`, `infra/**`, `ops/**` 변경시에만 자동 빌드/푸시. 태그(`v*`, `release-*`)와 수동 디스패치는 기존대로 유지.

### 런북 경로 통일
- `docs/agent_runbook_vps*.md`의 서버 경로를 `/srv/sogecon-app`로 정정(문서와 워크플로우 간 표기 일치).

...
- Web: Figma 디자인에 맞춰 메인 페이지 및 메뉴 리디자인 (`feat/homepage-menu-redesign`)
  - 헤더 로고 변경: `sogang_korean_logo.svg` 제거, 텍스트 "서강대학교 경제대학원 총동문회"로 변경
  - 드로워 상단 로고를 `sogang.svg` 이미지로 교체 (하드코딩된 SVG 제거)
  - 퀵 액션 타일 6개 재구성: 순서/색상/라벨을 Figma 디자인에 맞춰 업데이트
    * 동문 수첩(brand-primary), 행사 일정(brand-primaryDark), 갤러리(yellow-600)
    * 자유게시판(#b08968), 가입인사(teal-700), 동문 소식(slate-700)

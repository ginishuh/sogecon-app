...
- Build: OpenAPI 스키마 재생성 (댓글 API 추가)
  - packages/schemas/openapi.json, index.d.ts 업데이트
    * CommentCreate, CommentRead 스키마 추가
    * /comments/ 엔드포인트 추가 (GET, POST)
    * /comments/{comment_id} 엔드포인트 추가 (DELETE)
  - verify-dto CI 실패 수정: 스키마와 실제 API 동기화

- Build: CI 번들 크기 제한 완화 (1000KB → 1100KB)
  - .github/workflows/ci.yml 업데이트
  - 댓글 기능, DrawerMenu, CommentsSection 등 신규 기능 추가로 번들 크기 증가
  - 현재 번들 크기: 1010KB (동적 import 최적화 적용 후)
  - 향후 기능 추가를 위한 여유 공간 확보 (90KB 여유)

- Refactor: GitHub Copilot 코드 리뷰 반영 (PR #50)
  - 날짜 포맷팅 중복 제거
    * apps/web/lib/date-utils.ts: formatBoardDate, formatFullDate 유틸리티 함수 추출
    * apps/web/app/board/page.tsx: formatBoardDate 사용으로 중복 제거
    * apps/web/app/board/[id]/page.tsx: formatFullDate 사용
    * apps/web/components/comments-section.tsx: formatFullDate 사용
    * 날짜 포맷팅 로직이 3개 파일에 중복되어 있던 것을 단일 유틸리티로 통합
  - Next.js Router 사용으로 개선
    * apps/web/components/drawer-menu.tsx: window.location.href → useRouter().push('/login')
    * 전체 페이지 리로드 대신 클라이언트 사이드 라우팅 사용으로 성능 개선
  - 문서 정리
    * docs/worklog.md: 2025-10-06 섹션의 중복 엔트리 제거 (261-262줄)
  - 불필요한 방어 코드 제거
    * apps/api/routers/comments.py: member.id None 체크 제거
    * require_member 성공 시 member.id는 primary key로 항상 존재
    * unreachable code 제거로 코드 명확성 향상
  - SQLAlchemy ORM 비교 개선
    * apps/api/services/comments_service.py: getattr 안티패턴 제거
    * comment.author_id 직접 접근 + bool() 명시적 변환으로 변경
    * pyright SQLAlchemy Column 타입 문제 해결 (noqa 없이)
    * 타입 안전성 및 코드 가독성 향상
  - Docker 환경 감지 개선
    * apps/web/lib/api.ts: 명시적 환경변수 NEXT_PUBLIC_API_INTERNAL_URL 사용
    * 암시적 가정(NODE_ENV + localhost 체크) 대신 명시적 설정으로 변경
    * 유지보수성 및 명확성 향상

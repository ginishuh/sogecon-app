...
## Hotfix: DB 대기 타임아웃과 로그인/세션 통합

### 증상
- `make db-up` 실행 시 컨테이너는 기동되지만 "[db] Timeout waiting for postgres to be ready"로 종료(WSL2/저사양 첫 부팅시 빈번)
- 프론트/백엔드에서 관리자/멤버 로그인 흐름이 분리되어 혼선 및 유지보수 복잡도 증가

### 조치
1) Makefile 개선
- Health.Status(`healthy`) 대기 → 타임아웃 시 `pg_isready` 폴백
- 기본 타임아웃 90초(`WAIT_FOR_PG_TIMEOUT` 환경변수로 조정)

2) 인증/세션 통합
- `/auth/login` 단일 엔드포인트로 관리자/멤버 모두 처리(관리자 우선 매칭)
- 통합 세션 `user`(student_id, roles, id, email) 저장 + 레거시 키(admin/member) 동시 기록/삭제(호환)
- `/auth/session` 신설: { kind, student_id, email, id } 반환
- `/auth/logout` 및 `/auth/member/logout` 호출 시 모든 세션 키 정리
- `require_member`는 관리자도 통과(백엔드/프론트 가드 일치)

3) 프론트 정리
- 세션 조회 `getSession()` → `/auth/session` 단일화
- 헤더 표시: 이메일 → 학번(student_id)
- 로그인 페이지: 모드 토글(멤버/관리자) 제거, 단일 로그인 UI로 단순화
- `RequireMember`가 관리자도 허용

### 결과
- DB 컨테이너 콜드 스타트에서도 `make db-up` 신뢰성 향상
- 로그인/세션 관리 단순화 및 혼선 제거, UI 일관성 개선
- 개발 편의 및 운영 안정성 향상

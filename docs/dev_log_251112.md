...
**문제**:
- CI에서 `../../.venv/bin/python: not found` 에러 발생
- packages/schemas의 gen 스크립트가 로컬 .venv Python을 사용하려고 시도

**원인**:
- CI 환경에는 .venv가 없음
- dto-verify workflow에서 이미 `python scripts/export_openapi.py`로 OpenAPI 생성
- 이후 `pnpm run gen`을 실행하여 중복으로 OpenAPI 생성 시도

**해결**:
- `.github/workflows/dto-verify.yml` 수정: `gen` → `gen-dts`
- CI에서는 OpenAPI를 먼저 생성한 후 타입만 재생성
